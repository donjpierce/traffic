...
def init_custom_agent(graph, n=1, car_id=None, alternate_route=None):
    ...
    cars_frame["xbin"], cars_frame["ybin"] = models.determine_bins(
        graph.fig, cars_frame
    )
...
def init_custom_lights(fig_axis, prescale=None):
    ...
    light["out-xpositions"] = [
        x + epsilon * out_vectors[j][0] for j in range(light["degree"])
    ]
    light["out-ypositions"] = [
        y + epsilon * out_vectors[j][1] for j in range(light["degree"])
    ]
